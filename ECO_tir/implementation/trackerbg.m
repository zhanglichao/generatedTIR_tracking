function results = trackerbg(params)
...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            xt = extract_features_bg(im, bg_im, sample_pos, sample_scale, features, global_fparams, feature_extract_info);
    ...
    if seq.frame == 1
        ...
        xl = extract_features_bg(im, bg_im, sample_pos, currentScaleFactor, features, global_fparams, feature_extract_info);
    ...
    if strcmpi(seq.format, 'vot_local')
        results.res = seq.rect_position; results.frame = seq.frame;
        if seq.reset
            % from vot-toolkit experiment_supervised
            imgbounds = [size(im,2),size(im,1)];
            o = region_overlap(seq.rect_position(seq.frame,:), seq.gt(seq.frame,:), imgbounds);
            if o(1) <= 0
...
%     if seq.use_reinitialization  % detect failures and reinit
%         area = rectint(rect_position_vis, seq.gt(seq.frame,:));
%         if area < eps && seq.use_reinitialization
