function results = trackerbgcat(params)
...
bg_learning_rate = params.bg_learning_rate;
...
% Define data types
if params.use_gpu
    if isempty(global_fparams.gpu_id)
%         gpuDevice();
    elseif global_fparams.gpu_id > 0
%         gpuDevice(global_fparams.gpu_id);
    end
  %  params.data_type = zeros(1, 'single', 'gpuArray');
    params.data_type = gpuArray(single(0));
else
...
[features, global_fparams, feature_info] = init_features_bg(features, global_fparams, is_color_image, img_sample_sz, 'odd_cells');

% Set feature info
img_support_sz = feature_info.img_support_sz;
feature_sz = feature_info.data_sz(1:end-1,:);
feature_dim = feature_info.dim;
num_feature_blocks = length(feature_dim)-1;
...
if params.use_gpu
    ...
    for k = 1:num_feature_blocks % sample_dim(k)+sample_dim(end) plug motion maks into conv feature cell
        samplesf{k} = zeros(filter_sz(k,1),(filter_sz(k,2)+1)/2,sample_dim(k)+sample_dim(end),params.nSamples, 'like', params.data_type_complex);
    end
else
    for k = 1:num_feature_blocks
        samplesf{k} = zeros(params.nSamples,sample_dim(k)+sample_dim(end),filter_sz(k,1),(filter_sz(k,2)+1)/2, 'like', params.data_type_complex);
    end
end
...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            xt = extract_features_bgcat(im, bg_im, sample_pos, sample_scale, features, global_fparams, feature_extract_info);
    ...
    if seq.frame == 1
        ...
        xl = extract_features_bgcat(im, bg_im, sample_pos, currentScaleFactor, features, global_fparams, feature_extract_info);
    ...
    if train_tracker     
        ...
        if seq.frame == 1
            ...
            for k = 1:num_feature_blocks
                hf{1,1,k} = zeros([filter_sz(k,1) (filter_sz(k,2)+1)/2 sample_dim(k)+sample_dim(end)], 'like', params.data_type_complex);
            end
        else
            ...
            % add bg lichao bg_learning_rate=1, maybe no need bg_im
            bg_im = uint8((1 - bg_learning_rate) * single(bg_im) + bg_learning_rate * single(im));
        end
