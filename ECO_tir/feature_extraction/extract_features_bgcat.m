function feature_map = extract_features_bgcat(image, bg_image, pos, scales, features, gparams, extract_info)
...
for sz_ind = 1:num_sizes
    ...
    for scale_ind = 1:num_scales
        ...
        img_samples_bg{sz_ind}(:,:,:,scale_ind) = sample_patch(bg_image, pos, img_sample_sz*scales(scale_ind), img_input_sz, gparams);
    end
end
...
feature_map = cell(1, 1, num_feature_blocks-1); % substract bg block
...
%     gparams.cell_size = feat.fparams.cell_size;
    ...
    if feat.is_cell
        ...
        feat.fparams.diff_im = img_samples_bg{img_sample_ind};
%         num_blocks = 1; 
        feature_map{ind} = cat(3,feature_map{ind},feat.getFeature(img_samples{img_sample_ind}, feat.fparams, gparams));
    end
...
feature_map{1}(:,:,end,:) = feature_map{1}(:,:,end,:) * gparams.threweight;
end