...
cnn_params.nn_name = 'tirganpix_real-vgg-125-2.0-DCF-ftlr46-data-12345-s4_new_epo45.mat'; % Name of the network
...
params.bg_learning_rate = 1.0;
thresh_feature_scale = 5.0;
thresh_params.threshold = 0.5/25;
thresh_params.threshold_type = 'larger_than';
thresh_params.pre_mapping = @abs;
thresh_params.post_mapping = @(x) thresh_feature_scale * x;
thresh_params.nDim = 1;
thresh_params.cell_size = 4;
thresh_params.cell_selection_thresh = 1.0^2;
% Which features to include
params.t_features = {
    struct('getFeature',@get_cnn_layers, 'fparams',cnn_params),...
    struct('getFeature',@get_threshold, 'fparams',thresh_params)};
% increse get_threshold importance
params.t_global.threweight = 55;
...
params.gpu_id = [1];                     % Set the GPU id, or leave empty to use default

% Initialize
params.seq = seq;

% Run tracker
results = trackerbg(params);
